...
public class TagRepositoryMysql extends GenericRepositoryMysql<Tag> implements TagRepository {
    ...
    private void initializeTable() {
        ...
        this.executeUpdate("CREATE TABLE IF NOT EXISTS " + TABLE + "(" +
                ...
                KEY_FIELDS.split(",")[1] + " VARCHAR(20)," +
                KEY_FIELDS.split(",")[2] + " VARCHAR(20))");
    }

    @Override
    public Tag create(Tag tag) {
        String sql = String.format("INSERT INTO %s (%s) VALUES ('%s','%s')", TABLE, FIELDS,
    ...
    @Override
    protected Tag retriever(ResultSet resultSet) {
        try {
            return new Tag(
    ...
    public Optional<Tag> findByName(String name) {
        return this.executeQuery(String.format("SELECT %s FROM %s WHERE %s = '%s'", KEY_FIELDS, TABLE, KEY_FIELDS.split(",")[1], name)).stream()
                .findFirst();
    }
